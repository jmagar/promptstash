...
const remoteServerSchema = z.object({
  ...
  headers: z.record(z.string()).optional().describe('HTTP headers for authentication'),
});
  ...
  try {
    ...
    for (const [serverName, serverConfig] of Object.entries(servers)) {
      ...
      if ('command' in serverConfig) {
        ...
        if (!serverConfig.command.startsWith('/') && !serverConfig.command.startsWith('./')) {
          warnings.push(
      ...
      if ('url' in serverConfig) {
        ...
        if (serverConfig.url.startsWith('http://')) {
          warnings.push(`Server '${serverName}': Using HTTP instead of HTTPS may be insecure`);
        }

        // Warn about authentication
        if (!serverConfig.headers || !serverConfig.headers.Authorization) {
          warnings.push(`Server '${serverName}': No Authorization header configured`);
        }
      }
    }
...
export function validateMCPFile(content: string): {
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      ...
      errors.push(`Failed to parse: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    return { valid: false, errors, warnings };
  }
}
