...
export const agentFrontmatterSchema = z
  .object({
    ...
    tools: z.array(z.string()).optional().describe('Array of tool names the agent can use'),
    'allowed-tools': z.array(z.string()).optional().describe("Alternative to 'tools' field"),
    model: z
      .enum(MODEL_OPTIONS)
      .optional()
...
export function validateAgentFilename(filename: string): {
  ...
  if (!kebabCaseRegex.test(baseName)) {
    errors.push('Filename must be kebab-case (lowercase with hyphens, e.g., security-reviewer.md)');
  }
